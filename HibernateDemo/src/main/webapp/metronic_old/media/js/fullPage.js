...
function FullPage(options) {
	...
	goPage = function(to) {
		...
		//if (_isNav) navChange(indexOld, indexNowRef);
		setTimeout(function() {
		...
		setTimeout(function() {
			...
			replaceClass(page[indexNowRef], 'slide', 'current');

			if (options.callback) {
				options.callback(indexNowRef, page[indexNowRef]);
			}
		...
		(function(m) {
			...
			switch (true) {
				...
				(function() {
					...
					var navId = m.split(':')[1],
						navNameExt = m.split(':')[1],
						navObj = document.getElementById(navId),
						navObjByNameExt = document.getElementsByName(navId),
						navLenExt,
						gotoPage,
						_t;

					navChildren = navObjByNameExt;
					
					navLenExt = navObjByNameExt.length;
					...
					while (navLenExt--) {
						...
						navChildren[navLenExt].setAttribute('data-page', navLenExt);
					}
					...
					for (var it=0; it<navChildren.length; it++)
					{
						onTap(navChildren[it], gotoPage, 1);
					}
